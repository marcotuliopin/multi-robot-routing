...
from rewards import rpositions, rvalues
from scipy.spatial.distance import cdist
from utils import calculate_mutation_probability
import ga
import plots
import numpy as np
import random
...
toolbox.register("individual", ga.init_individual, creator.Individual)
...
dist_mx = cdist(rpositions, rpositions, metric="euclidean")
...
def main(toolbox, seed=None):
    ...
    for ind in population:
        ind.fitness.values = toolbox.evaluate(ind, 0, NGEN, rvalues, dist_mx, MAXD)
    ...
    for gen in range(1, NGEN):
        ...
        for ind in offspring:
            ind.fitness.values = toolbox.evaluate(ind, gen, NGEN, rvalues, dist_mx, MAXD)
...
if __name__ == "__main__":
    ...
    if args.plot_path:
        ...
        plots.plot_path(ax, rpositions, individual[0], color='orange')
        plots.plot_path(ax, rpositions, individual[1], color='green')
    ...
    if args.plot_distances:
        plots.plot_distances(individual[0], individual[1])
        if args.save_plot:
